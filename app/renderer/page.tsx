...
import { toast } from "sonner"
 ...
 function RenderContent() {
  const { componentType, setComponentType,state } = useComponentContext();
...
function copyToClipBoard(codeToCopy : string){
  navigator.clipboard.writeText(codeToCopy)
  .then(() =>{
    toast("Code has been Copied")
  })
  .catch(() =>{
    toast("Error while copying code")
  })
}
  ...
  const renderCode = () =>{
    switch(componentType){
      case "Button" :
        const code = MainCode(state.button);
  ...
  return (
    ...
    <div className="max-w-[1220px] w-full flex flex-col items-center">
      <div className="w-full flex flex-col md:flex-row justify-center items-stretch mb-5">
        ...
        <Card className="w-full max-w-[600px] min-h-[700px]">
          <CardHeader>
